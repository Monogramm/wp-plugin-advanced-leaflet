...
class WP_Plugin_Advanced_Leaflet_Geocoder
{
	/**
	...
	public function __construct ($address) {
		...
		$geocoder = $settings_storage->get('geocoder');

		$cached_address = 'wppt_leaflet_' . $geocoder . '_' . $address;

		/* retrieve cached geocoded location */
		$found_cache = $settings_storage->get($cached_address );
		...
		if ( $found_cache ) {
			...
			try {
				...
				$locations = $settings_storage->get('leaflet_geocoded_locations', array());
				array_push($locations, $cached_address);
				update_option('wppt_leaflet_geocoded_locations', $locations);
			} catch (Exception $e) {
				// failed
	...
	public static function remove_caches () {
		$addresses = get_option('wppt_leaflet_geocoded_locations', array());
		foreach ($addresses as $address) {
			delete_option($address);
		}
		delete_option('wppt_leaflet_geocoded_locations');
	}
	...
	private function google_geocode ( $address ) {
		...
		$key = $settings_storage->get('google_appkey');
