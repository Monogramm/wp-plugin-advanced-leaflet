...
require_once LEAFLET_MAP__PLUGIN_DIR . 'includes/lib/shortcodes/class.shortcode.php';
...
class Leaflet_Map_Shortcode extends Leaflet_Shortcode
{
    ...
    protected function getAtts($atts='')
    {
        ...
        $atts['zoom'] = array_key_exists('zoom', $atts) ?
            $zoom : $settings_storage->get('default_zoom');
        $atts['height'] = empty($height) ?
			$settings_storage->get('default_height') : $height;
        $atts['width'] = empty($width) ? $settings_storage->get('default_width') : $width;
        $atts['zoomcontrol'] = isset($zoomControl)
            ? $zoomControl
            : (array_key_exists('zoomcontrol', $atts)
                ? $zoomcontrol
                : $settings_storage->get('show_zoom_controls'));
        $atts['min_zoom'] = array_key_exists('min_zoom', $atts) ?
            $min_zoom : $settings_storage->get('default_min_zoom');
        $atts['max_zoom'] = empty($max_zoom) ?
			$settings_storage->get('default_max_zoom') : $max_zoom;
        $atts['scrollwheel'] = isset($scrollWheelZoom)
            ? $scrollWheelZoom
            : (array_key_exists('scrollwheel', $atts)
                ? $scrollwheel
                : $settings_storage->get('scroll_wheel_zoom'));
        $atts['doubleclickzoom'] = array_key_exists('doubleclickzoom', $atts) ?
            $doubleclickzoom : $settings_storage->get('double_click_zoom');

        // @deprecated backwards-compatible fit_markers
        $atts['fit_markers'] = array_key_exists('fit_markers', $atts) ?
            $fit_markers : $settings_storage->get('fit_markers');
        ...
        */
        if (!array_key_exists('attribution', $atts)) {
            $atts['attribution'] = $settings_storage->get('default_attribution');
        }
        ...
        // filter out nulls
        $map_options = WP_Plugin_Advanced_Leaflet::filter_null($map_options);
        ...
        foreach($map_options as $key=>$val) {
            ...
            $liquid = WP_Plugin_Advanced_Leaflet::liquid($original_value);
        ...
        $atts['raw_map_options'] = WP_Plugin_Advanced_Leaflet::rawDict($raw_map_options);

        $tile_layer_options = array(
            'tileSize' => empty($tilesize) ? $settings_storage->get('tilesize') : $tilesize,
            'subdomains' => empty($subdomains) ? $settings_storage->get('map_tile_url_subdomains') : $subdomains,
            'id' => empty($mapid) ? $settings_storage->get('mapid') : $mapid,
            'accessToken' => empty($accesstoken) ? $settings_storage->get('accesstoken') : $accesstoken,
            'zoomOffset' => empty($zoomoffset) ? $settings_storage->get('zoomoffset') : $zoomoffset,
            'noWrap' => filter_var(empty($nowrap) ? $settings_storage->get('tile_no_wrap') : $nowrap, FILTER_VALIDATE_BOOLEAN),
        );

        $tile_layer_options = WP_Plugin_Advanced_Leaflet::filter_empty_string($tile_layer_options);
        $tile_layer_options = WP_Plugin_Advanced_Leaflet::filter_null($tile_layer_options);
...
?>;"></div><?php
    ...
    protected function getHTML($atts='', $content=null)
    {
        ...
        if (!empty($address)) {
            ...
            $location = new WP_Plugin_Advanced_Leaflet_Geocoder($address);
            $lat = $location->lat;
            $lng = $location->lng;
        }
        ...
        map uses lat/lng
        */
        $lat = empty($lat) ? $settings_storage->get('default_lat') : $lat;
        $lng = empty($lng) ? $settings_storage->get('default_lng') : $lng;
        ...
        */
        if (wp_script_is('leaflet_mapquest_plugin', 'registered')) {
            ...
            $tileurl = empty($tileurl) ? $settings_storage->get('map_tile_url') : $tileurl;
        }

        $detect_retina = empty($detect_retina) ? $settings_storage->get('detect_retina') : $detect_retina;
...
?>);<?php
        ...
        $show_scale = isset($show_scale) ? $show_scale : $settings_storage->get('show_scale');
